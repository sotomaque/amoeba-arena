...
export function FinalResults({ gameState, playerId }: FinalResultsProps) {
	...
	const fireConfetti = useCallback(async () => {
		...
		// Dynamic import - only load canvas-confetti when needed (reduces initial bundle)
	...
	const floatingElements = (index: number) =>
		shouldReduceMotion
			? {}
			: {
					animate: {
	...
	const hoverScale = shouldReduceMotion ? {} : { whileHover: { scale: 1.02 } };
				...
				<motion.div className="ghibli-card p-6 text-center" {...fadeInUp(0.5)}>
					...
					<p className="text-muted-foreground mt-2">
						{playerRank <= 3
							...
							: "Keep practicing those survival skills!"}
					</p>
				</motion.div>
			)}
			...
			<motion.div className="ghibli-card p-6" {...fadeInUp(0.7)}>
				...
				<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
					{[
						...
						{
							label: "Total Players",
							value: gameState.players.filter((p) => !p.isHost).length,
							icon: "👥",
						},
						{
							label: "Survivors",
							value: gameState.players.filter(
								(p) => !p.isEliminated && !p.isHost,
							).length,
							icon: "🌿",
						},
						{
							...
							value: Math.max(
								...gameState.players.map((p) => p.population),
							).toLocaleString(),
							icon: "📈",
						},
					].map((stat, index) => (
						<motion.div
							...
							animate={{ opacity: 1, y: 0 }}
							transition={
								shouldReduceMotion ? undefined : { delay: 0.8 + index * 0.1 }
							}
						>
